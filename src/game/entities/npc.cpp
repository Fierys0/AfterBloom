...
void NpcEntity::Spawn(Vector2 position, Vector2 size) {
  ...
  npcFont = Fumbo::Assets::LoadFont("assets/fonts/Montserrat-ExtraBold.ttf", 24);
...
void NpcEntity::StartLine(int idx) {
  dialogueIndex = idx;
  vn->SetSpeaker(label, Color{100, 160, 60, 255}); // Match prologue style green for NPC
...
void NpcEntity::Update(Vector2 playerPos, bool interactPressed) {
  ...
  if (playerNear && (IsKeyPressed(KEY_E) || interactPressed) && !dialogue.empty()) {
    talking = true;
    StartLine(0);
  }
}
...
void NpcEntity::DrawWorld() const {
  ...
  if (playerNear && !talking) {
    DrawText(TextFormat("[E] Talk to %s", label.c_str()),
             (int)spawnPos.x - 30, (int)spawnPos.y - 50, 16, WHITE);
  }
}
...
void NpcEntity::DrawHUD() const {
  if (!talking || dialogue.empty()) return;
